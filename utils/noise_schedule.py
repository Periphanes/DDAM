...
def get_noise(config, sigma_min=None, sigma_max=None, dtype=torch.float32):
  if config == 'geometric':
    return GeometricNoise(sigma_min,
                          sigma_max)
  elif config == 'loglinear':
    return LogLinearNoise()
  elif config == 'cosine':
    return CosineNoise()
  elif config == 'cosinesqr':
    return CosineSqrNoise()
  elif config == 'linear':
    return Linear(sigma_min,
                  sigma_max,
                  dtype)
  else:
